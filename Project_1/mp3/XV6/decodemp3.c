...
void III_huffman_decode(long int is[SBLIMIT][SSLIMIT], struct III_side_info_t *si, int ch, int gr, int part2_start, struct frame_params *fr_ps)
{
    ...
    // printf(1, "before init huffman\n");
    initialize_huffman();
    // printf(1, "after init huffman\n");
...
void read_syn_window(double window[HAN_SIZE])
{
    ...
    {
        read(fp, t, 150);
        i_sscanf(t, "D[%d] = %f D[%d] = %f D[%d] = %f D[%d] = %f\n",
                 j, f, j + 1, f + 1, j + 2, f + 2, j + 3, f + 3);
        printf(1, "D[%d] = %f D[%d] = %f D[%d] = %f D[%d] = %f\n",
                 *j, *f, *(j + 1), *(f + 1), *(j + 2), *(f + 2), *(j + 3), *(f + 3));
        if (i == j[0])
        {
            window[i] = f[0];
...
void out_fifo(short pcm_sample[2][SSLIMIT][SBLIMIT], int num, struct frame_params *fr_ps, int done, int outFile, unsigned long *psampFrames)
{
    int i, j, l;
    int stereo = fr_ps->stereo;
